class Cart {
    cartItem= undefined;
     #loadCart() {
      this.cartItem = JSON.parse(localStorage.getItem(`${this.key}`)) || [
        {
 ...
 checkout() {
      document.querySelector(
        ".js-checkout"
      ).innerHTML = `${this.cartItem.length} : Items`;
    };
 ...
 savecart() {
      localStorage.setItem(`${this.key}`, JSON.stringify(this.cartItem));
    };
...
let busnniescart = new Cart('busnniescart');
